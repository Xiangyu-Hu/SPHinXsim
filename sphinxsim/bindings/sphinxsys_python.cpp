  ...
  if (py::isinstance<py::array>(input)) {
    ...
    if (arr.size() != 2) {
      throw py::value_error(
          "Vector must have exactly 2 elements for 2D simulation");
    }
...
void safe_createDomain(SPHSimulation &sim, const py::object &domain_dimensions,
                       Real particle_spacing) {
  Vecd dims = convert_to_vecd(domain_dimensions);
  sim.createDomain(dims, particle_spacing);
}

void safe_defineDomain(SPHSimulation &sim, const py::object &domain_dimensions,
                       Real particle_spacing) {
  Vecd dims = convert_to_vecd(domain_dimensions);
...
void safe_addObserver_single(SPHSimulation &sim, const std::string &name,
                             const py::object &position) {
  Vecd pos = convert_to_vecd(position);
  sim.addObserver(name, pos);
}

FluidBlockBuilder &safe_block(FluidBlockBuilder &builder,
                              const py::object &dimensions) {
  Vecd dims = convert_to_vecd(dimensions);
  return builder.block(dims);
}

WallBuilder &safe_hollowBox(WallBuilder &builder,
                            const py::object &domain_dimensions,
                            Real wall_width) {
  Vecd dims = convert_to_vecd(domain_dimensions);
