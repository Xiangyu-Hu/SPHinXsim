...
class TestLoadConfigHelper:
    ...
    def test_valid_returns_config_and_zero(self, tmp_path):
        p = self._write(tmp_path, self._valid_data())
        config, rc = _load_config(p)
        assert rc == 0
        assert config is not None
        assert config.name == "helper test"

    def test_missing_file_returns_none_and_nonzero(self, tmp_path):
        config, rc = _load_config(tmp_path / "missing.json")
        assert config is None
        assert rc != 0

    def test_bad_json_returns_none_and_nonzero(self, tmp_path):
        p = tmp_path / "bad.json"
        p.write_text("{{{{")
        config, rc = _load_config(p)
        assert config is None
    ...
    def test_invalid_schema_returns_none_and_nonzero(self, tmp_path):
...
class TestCLIGenerate:
    ...
    def test_generate_creates_parent_dirs(self, tmp_path):
        out_file = tmp_path / "nested" / "dir" / "cfg.json"
        rc = main(["generate", "water flow", "-o", str(out_file)])
        assert rc == 0
        assert out_file.exists()

    def test_generate_oserror_returns_nonzero(self, tmp_path, capsys):
        out_file = tmp_path / "cfg.json"
